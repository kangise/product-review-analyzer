    ...
    def _parse_opportunities(self):
        ...
        if opp_section:
            ...
            for rank, title, scenario_brief, details_text in matches:
                # 提取详细信息
                solution_match = re.search(r'\*\*创新方案\*\*[：:]?\s*(.+?)(?=\*\*|$)', details_text, re.DOTALL)
                value_match = re.search(r'\*\*预期价值\*\*[：:]?\s*(.+?)(?=\*\*|$)', details_text, re.DOTALL)
                
                opportunities.append({
                    "rank": int(rank),
                    "title": title.strip(),
                    "scenario": scenario_brief.strip(),
                    "solution": solution_match.group(1).strip() if solution_match else "提供创新解决方案",
